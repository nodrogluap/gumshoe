...
#'
#' @param sleuth_obj An existing Sleuth object as generated by sleuth_prep() and fit by sleuth_fit()
#' @param all_data A Boolean value that indicates whether all results of the Wald test should be assigned to a variable. Defaults to TRUE.
#' @param sig_data A Boolean value that indicates whether all the significant results of the Wald test coupled with fdr_cutoff should be assigned to a variable. Defaults to TRUE.
...
sleuth_object_result <- function(sleuth_obj, all_data = TRUE, sig_data = TRUE, q_max = .05, test = "wt") {
  coeff <- colnames(sleuth_obj$design_matrix)
  sleuth_obj_name <- deparse(substitute(sleuth_obj))
...
ensembl_to_id <- function(sig_results, entire_gene_name = TRUE){
  if (!exists("mart")){
    cat("Function requires a BioMart database and dataset. Run the following command to create the required `mart` object:\n")
    cat("\n")
