...
sleuth_kruskal_wallis <-
  function(sleuth_obj,
    ...
    if (grouping %in% colnames(s2c_df)) {
      scaled_trancript_counts$sample <-
        paste(scaled_trancript_counts$sample, grouping, sep = "_")
    } else {
...
#' @param sleuth_obj An existing Sleuth object as generated by sleuth_prep() and fit by sleuth_fit().
#'
#' @return Dataframe with all the RSS values for a given sleuth_object.
#' @export
#' @examples
#' # Given a Sleuth object, retrieve the RSS values.
#' sleuth_model_rss(so_nominal)
sleuth_model_rss <- function(sleuth_obj) {
