...
# TODO: Change the colours for the groups at the top
# TODO: Permit greater genes to be showcased (current max is 3)
# TODO: Allow for all the transcript p-values to be shown 
# TODO: Cluster by each individual transcript and then with them (ie group all the transcripts for a specific gene and cluster within them, which would help with viewing the data)
...
#' @param q_max The maximum q-value that should be used as a cutoff to determine significant transcripts
#' @param q_marker_vals A vector of the q-value cutoffs that should be used.
...
#' heatmap_plot(so, genes = c("Tp53", "Tnf", "Egfr", "Egr1"), q_max = .01, q_marker_vals = c(.05, .01, .001), test = "wt", bloxplot = FALSE)
heatmap_plot <- function(sleuth_obj, genes, q_max = .05, q_marker_vals = c(.05, .01, .001), test = "wt", boxplot = TRUE){
  ...
  transcript_gene_pair <- unique(scaled_trancript_counts[,c('target_id', 'ext_gene')])
  # assign("transcript_gene_pair", transcript_gene_pair, envir = .GlobalEnv)
  ...
  # assign("transcript_p_val_1", transcript_p_val, envir = .GlobalEnv)
  ...
  # assign("transcript_p_val_2", transcript_p_val, envir = .GlobalEnv)
  
  # Determine what symbols should be assigned to the significant markers
  pch <- transcript_p_val
  annotation_symbol <- letters[1:length(q_marker_vals)]
  q_marker_vals <- sort(q_marker_vals)
  for (cutoff in q_marker_vals){
    pch[pch < as.numeric(cutoff)] <- annotation_symbol[match(cutoff, q_marker_vals)]
  }
  ...
  legend_label <- paste("<", q_marker_vals, sep=" ")
  lgd_sig = Legend(pch = annotation_symbol, type = "points", labels = legend_label, border = TRUE)
