...
volc_plot <- function(df, labels = FALSE, label_column_name = "external_gene_name", 
                      x_axis_column_name = "b", y_axis_column_name = "qval", 
                      x_cutoff = 2, y_cutoff = -log10(.05), 
  ...
  if (!"ggrepel" %in% (.packages())) {
    ...
    if (labels) {
      ...
      if (any("Not sig." %in% df$diff_expressed)) {
        ggplot(df, aes(x = log2FC, y = Y_axis, col = diff_expressed, label = label)) +
...
heatmap_plot <- function(sleuth_obj, genes, q_max = .05, test = "wt", iqf = 0, sig_marker_values = c(.05, .01, .001), sig_markers = c("*", "**", "***"),
  ...
  if (sleuth_obj$gene_mode == TRUE) {
    ...
    scaled_transcript_counts$target_id <- paste(scaled_transcript_counts$ext_gene, scaled_transcript_counts$target_id, sep = " - ")
  ...
  transcript_p_val <- merge(transcript_p_val, scaled_transcript_counts, by = "row.names", all.y = TRUE)
  ...
  if (boxplot) {
    ...
    if (sleuth_obj$gene_mode == TRUE){
      ...
      ha_c <- Heatmap(scaled_transcript_count_matrix,
                      ...
                      row_title = "Genes", row_title_rot = 0, row_names_max_width = max_text_width(rownames(scaled_transcript_count_matrix), gp = gpar(fontsize = 12)),
      ...
      ha_c <- Heatmap(scaled_transcript_count_matrix,
                      ...
                      row_title = "Transcripts", row_title_rot = 0, row_names_max_width = max_text_width(rownames(scaled_transcript_count_matrix), gp = gpar(fontsize = 12)),
                      show_column_names = FALSE,
                      na_col = "black",
                      top_annotation = ha_top, left_annotation = ha_left, right_annotation = ha_right)
    }
