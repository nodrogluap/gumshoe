...
# !!! The following function has been replaced by slueth_model_rss
sleuth_model_basic_rss <- function(sleuth_obj) {
...
targetid_u_test_quantitative <- function(sleuth_obj, tid, factor_name, value_threshold) {
  ...
  u <- wilcox.test(tpm ~ u_group_name, data = d)
  c(tid, u$p.value)
}
...
sleuth_oneway_anova <- function(sleuth_obj, factor_name, target_ids = unique(sleuth_obj$obs_norm_filt$target_id)) {
  oneway_results_list <- lapply(target_ids, function(target_id) {
    targetid_one_way_anova_test(sleuth_obj, target_id, factor_name)})
...
sleuth_kruskal_wallace <- function(sleuth_obj, factor_names, target_ids = unique(sleuth_obj$obs_norm_filt$target_id)) {
  kw_results_list <- lapply(target_ids, function(target_id) {
    targetid_single_factor_kruskal_wallace_test(sleuth_obj, target_id, factor_names)})
...
targetid_paired_t_test <- function(sleuth_obj, tid, pair_name, factor_name) {
  ...
  t <- t.test(x = log1p(row_by_factor_level[[1]]$est_counts),
              y = log1p(row_by_factor_level[[2]]$est_counts),
              paired = TRUE, alternative = "two.sided")
  logFCs <- log(row_by_factor_level[[2]]$est_counts / row_by_factor_level[[1]]$est_counts)
  shapiro <- ifelse(sum(!is.nan(logFCs)) < 3, NA,
                    ifelse(length(unique(logFCs)) < 3, NA, shapiro.test(logFCs)$p.value))
...
sleuth_paired_t <- function(sleuth_obj, pair_name, factor_name, target_ids = unique(sleuth_obj$obs_norm_filt$target_id)) {
  t_test_results_list <- lapply(target_ids, function(target_id) {
    targetid_paired_t_test(sleuth_obj, target_id, pair_name, factor_name)})
  ...
  colnames(t_test_results_dataframe) <- c("target_id", "pval", "qval", "b", 
...
replicate_dotplots <- function(sleuth_obj, design_matrix_combo) {
  ...
  for (sample1 in rownames(samples)) {
    for (sample2 in rownames(samples)) {
      plot(data$log_est_counts[which(data$sample == sample1)],
           data$log_est_counts[which(data$sample == sample2)],
           xlab = sample1, ylab = sample2, xlim = c(0, 12), ylim = c(0, 12))
    }
  }
}
