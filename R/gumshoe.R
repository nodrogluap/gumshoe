...
sleuth_prep_transcript_dosage <-
  function(sleuth_obj, metadata, transcript_id) {
    ...
    new_model <-
      as.formula(paste0(deparse(sleuth_obj$full_formula), " + ", transcript_id))
    sleuth_prep(new_metadata, new_model)
  }

targetid_u_test_quantitative <-
  function(sleuth_obj,
           tid,
           factor_name,
           value_threshold) {
...
targetid_single_factor_kruskal_wallace_test <-
  function(sleuth_obj, tid, factor_name) {
    d <- sleuth_obj$obs_norm_filt %>%
      mutate(est_counts = est_counts * sleuth_obj$est_counts_sf) %>%
      filter(target_id == tid) %>%
      left_join(sleuth_obj$sample_to_covariates, by = "sample")
...
sleuth_u <-
  function(sleuth_obj,
           factor_name,
           value_threshold,
           target_ids = unique(sleuth_obj$obs_norm_filt$target_id)) {
    U_results_list <-
      lapply(target_ids, function(target_id) {
        targetid_u_test_quantitative(sleuth_obj, target_id, factor_name, value_threshold)
      })
    U_results_dataframe <-
      as.data.frame(do.call(rbind, U_results_list))
    U_results_dataframe[, 3] <-
      p.adjust(U_results_dataframe[, 2], method = "fdr")
    colnames(U_results_dataframe) <- c("target_id", "pval", "qval")
    U_results_dataframe$pval <-
      as.numeric(as.character(U_results_dataframe$pval))
...
sleuth_reliable_target_ids <-
  function(sleuth_obj, min_count_per_sample = 1) {
...
sleuth_oneway_anova <-
  function(sleuth_obj,
    ...
    oneway_results_dataframe <-
      as.data.frame(do.call(rbind, oneway_results_list))
    oneway_results_dataframe[, 3] <-
      p.adjust(oneway_results_dataframe[, 2], method = "fdr")
    colnames(oneway_results_dataframe) <-
      c("target_id", "pval", "qval")
    oneway_results_dataframe$pval <-
      as.numeric(as.character(oneway_results_dataframe$pval))
    oneway_results_dataframe[order(oneway_results_dataframe$pval), ]
  }

sleuth_kruskal_wallace <-
  function(sleuth_obj,
           factor_names,
           target_ids = unique(sleuth_obj$obs_norm_filt$target_id)) {
    kw_results_list <-
    ...
    kw_results_dataframe <-
      as.data.frame(do.call(rbind, kw_results_list))
    kw_results_dataframe[, 3] <-
      p.adjust(kw_results_dataframe[, 2], method = "fdr")
    colnames(kw_results_dataframe) <- c("target_id", "pval", "qval")
    kw_results_dataframe$pval <-
      as.numeric(as.character(kw_results_dataframe$pval))
    kw_results_dataframe[order(kw_results_dataframe$pval), ]
  }

targetid_paired_t_test <-
  function(sleuth_obj, tid, pair_name, factor_name) {
    ...
    row_by_factor_level[[1]] <-
      row_by_factor_level[[1]][order(as.character(pull(row_by_factor_level[[1]], pair_name))), ]
    row_by_factor_level[[2]] <-
      row_by_factor_level[[2]][order(as.character(pull(row_by_factor_level[[2]], pair_name))), ]
    #print(row_by_factor_level[[1]]);
    #print(row_by_factor_level[[2]]);
    # This t-test assumes log-norml distribution
    t <-
      t.test(
        x = log1p(row_by_factor_level[[1]]$est_counts),
        y = log1p(row_by_factor_level[[2]]$est_counts),
        paired = TRUE,
        alternative = "two.sided"
      )
    logFCs <-
      log(row_by_factor_level[[2]]$est_counts / row_by_factor_level[[1]]$est_counts)
    shapiro <-
      ifelse(sum(!is.nan(logFCs)) < 3,
             NA,
...
sleuth_paired_t <-
  function(sleuth_obj,
    ...
    t_test_results_dataframe <-
      as.data.frame(do.call(rbind, t_test_results_list))
    t_test_results_dataframe[, 3] <-
      p.adjust(t_test_results_dataframe[, 2], method = "fdr")
    colnames(t_test_results_dataframe) <-
      c("target_id",
        "pval",
        "qval",
        "b",
    ...
    rownames(t_test_results_dataframe) <-
      t_test_results_dataframe$target_id
    # Otherwise NAs cause p-value to be considered as a factor column
    t_test_results_dataframe$pval <-
      as.numeric(as.character(t_test_results_dataframe$pval))
...
replicate_dotplots <- function(sleuth_obj, design_matrix_combo) {
  ...
  for (column_index in 1:length(design_matrix_combo)) {
    ...
    samples <-
      samples[which(samples[, column_index + 1] == design_matrix_combo[column_index]), ]
  }
  num_replicates <- dim(samples)[1]
  data <-
    sleuth_obj$obs_norm_filt[which(sleuth_obj$obs_norm_filt$sample %in% rownames(samples)), ]
  ...
  for (sample1 in rownames(samples)) {
    for (sample2 in rownames(samples)) {
      plot(
        data$log_est_counts[which(data$sample == sample1)],
